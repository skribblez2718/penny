---
name: synthia
description: Combine multiple distinct elements into a single, unified new product or concept — the opposite of analysis. Use for report generation, thematic synthesis, pattern connection, multi-source integration, or creating coherent narratives from diverse inputs. Do not use for exploration (echo), individual source analysis (annie), planning (piper), or critique (carren).
