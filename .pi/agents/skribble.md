---
name: skribble
description: Bring something into existence — generating, writing, or producing files from specifications. Use for file creation, code generation, document production, scaffolding, or content generation from designs. Do not use for exploration (echo), planning (piper), critique (carren), or task breakdown (tabitha).
tools: read, grep, find, ls, write, edit, bash, memory_smart_search, memory_add_drawer, memory_check_duplicate, memory_kg_add
