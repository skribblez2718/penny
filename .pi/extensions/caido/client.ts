...
export function classifyCaidoError(err: Error): CaidoErrorClassification {
  ...
  if (msg.includes("ECONNREFUSED") || msg.includes("ENOTFOUND")) {
    return {
      ...
      userMessage: "Cannot connect to Caido at the configured URL. Please verify Caido is running and the URL is correct.",
      retryable: true,
    };
  }
  if (msg.includes("not ready")) {
    return {
      category: "NOT_READY",
      userMessage: "Caido is not ready. It may still be initializing. Please wait a moment and try again.",
      retryable: true,
    };
  }
  ...
  if (msg.includes("timeout") || msg.includes("ETIMEDOUT")) {
    return {
      category: "TIMEOUT",
      userMessage: "Request to Caido timed out. Please check your network connection or try again later.",
      retryable: true,
    };
  }
  return {
...
export class InMemoryTokenCache implements TokenCache {
  ...
  async load(): Promise<{ accessToken: string; refreshToken?: string; expiresAt?: string } | undefined> {
    return this._token ?? undefined;
  }

  async save(token: { accessToken: string; refreshToken?: string; expiresAt?: string }): Promise<void> {
    this._token = token;
  }
