...
import { caidoInfoImpl } from "./tools/info.js";
import { caidoSearchImpl } from "./tools/search.js";
import { caidoRequestImpl } from "./tools/request.js";
import { caidoInterceptImpl } from "./tools/intercept.js";
import { caidoScopesImpl } from "./tools/scopes.js";
import { caidoFiltersImpl } from "./tools/filters.js";
import { caidoEnvironmentsImpl } from "./tools/environments.js";
import { caidoFindingsImpl } from "./tools/findings.js";
import { caidoSessionsImpl } from "./tools/sessions.js";
import { caidoCollectionsImpl } from "./tools/collections.js";
import { caidoEditImpl } from "./tools/edit.js";
import { caidoSendImpl } from "./tools/send.js";
import { caidoFuzzImpl } from "./tools/fuzz.js";
import { caidoProjectsImpl } from "./tools/projects.js";
import { caidoTasksImpl } from "./tools/tasks.js";
import { caidoFilesImpl } from "./tools/files.js";
...
export default async function caidoExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "caido_info",
    label: "Caido Info",
    description: "Fetch Caido instance health, current user (viewer), or installed plugin packages. Use mode=health as a smoke test.",
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_info", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      limit: Type.Optional(Type.Number({ description: "Max results (1-500, default 50)", minimum: 1, maximum: 500 })),
      ...
      recent_only: Type.Optional(Type.Boolean({ description: "Return most recent requests, ignoring filter" })),
    }),
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_search", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    name: "caido_request",
    label: "Caido Request",
    description: "Get full request/response details, response only, or generate a curl command for a request by ID.",
    ...
    parameters: Type.Object({
      ...
      mode: Type.Optional(Type.String({ description: "Mode: full | response | curl (default full)" })),
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_request", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_intercept", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      scope_id: Type.Optional(Type.String({ description: "Scope ID (required for update/delete)" })),
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_scopes", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      action: Type.String({ description: "Action: list | create | update | delete" }),
      filter_id: Type.Optional(Type.String({ description: "Filter ID (required for update/delete)" })),
      name: Type.Optional(Type.String({ description: "Filter name (required for create)" })),
      query: Type.Optional(Type.String({ description: "Filter query / clause (required for create)" })),
      alias: Type.Optional(Type.String({ description: "Optional alias for the filter" })),
    }),
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_filters", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_environments", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_findings", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_sessions", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      action: Type.String({ description: "Action: list | create | rename | delete" }),
      limit: Type.Optional(Type.Number({ description: "Limit for list (default 50)" })),
      collection_id: Type.Optional(Type.String({ description: "Collection ID (for rename/delete)" })),
      name: Type.Optional(Type.String({ description: "Collection name (for create/rename)" })),
    }),
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_collections", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    description: "Fetch a request, modify it (method, path, headers, body, replacements), and replay it. Returns the replay result.",
    ...
    parameters: Type.Object({
      ...
      set_headers: Type.Optional(Type.Array(Type.String({ description: "Headers to add/replace (e.g. 'X-Header: value')" }))),
      remove_headers: Type.Optional(Type.Array(Type.String({ description: "Header names to remove" }))),
      body: Type.Optional(Type.String({ description: "Replace request body" })),
      replacements: Type.Optional(Type.Array(Type.String({ description: "String replacements as 'from:::to'" }))),
      max_body_lines: Type.Optional(Type.Number({ description: "Max body lines (default 200)" })),
      max_body_chars: Type.Optional(Type.Number({ description: "Max body chars (default 5000)" })),
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_edit", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      raw_request: Type.Optional(Type.String({ description: "Raw HTTP request text (for raw mode)" })),
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_send", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    name: "caido_fuzz",
    label: "Caido Fuzz",
    description: "Create an automate session from a request and start a fuzzing task. Returns task ID immediately (does not wait for completion).",
    ...
    parameters: Type.Object({
      ...
      automate_session_id: Type.Optional(Type.String({ description: "Existing automate session ID" })),
      payloads: Type.Optional(Type.Array(Type.String({ description: "Optional payload hints (informational)" }))),
    }),
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_fuzz", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_projects", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Use action=cancel with task_id.",
    ],
    parameters: Type.Object({
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_tasks", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use action=list to see hosted files.",
      "Use action=delete with file_id.",
    ],
    parameters: Type.Object({
    ...
    async execute(_toolCallId: string, params: any) {
      return withCaidoClient("caido_files", config, { acquireSemaphore, releaseSemaphore, logger }, async () => {
        const client = await getClient();
