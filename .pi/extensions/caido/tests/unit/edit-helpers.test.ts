...
describe("modifyHeaders", () => {
  ...
  it("combines remove and set", () => {
    ...
    const result = modifyHeaders(headers, ["Old"], ["New: val"]);    expect(result).toEqual(["Host: x", "Keep: yes", "New: val"]);
  });
...
describe("buildModifiedRaw", () => {
  ...
  it("builds LF request with body", () => {
    const result = buildModifiedRaw("POST /api HTTP/1.1", ["Content-Type: json"], '{"k":"v"}', "\n");
