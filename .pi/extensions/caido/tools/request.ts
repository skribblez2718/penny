...
export async function caidoRequestImpl(params: CaidoRequestParams, client: Client): Promise<unknown> {
  ...
  if (mode === "response") {
    const result = await client.request.get(params.request_id, { requestRaw: false, responseRaw: true });
    if (!result) throw new Error(`Request ${params.request_id} not found`);
  ...
  if (result.response) {
    ...
    if (result.response.raw) {
      (output.response as any).raw = formatHttpRaw(decodeRaw(result.response.raw), opts);
    }
  }
