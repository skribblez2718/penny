import type { Client, ReplaySendResult } from "@caido/sdk-client";
import { decodeRaw, formatHttpRaw } from "../output.js";
import { CREATE_REPLAY_SESSION_RAW } from "../graphql.js";
import type { CreateReplaySessionRawResult, CreateReplaySessionRawVars } from "../graphql.js";
...
export async function caidoSendImpl(params: CaidoSendParams, client: Client): Promise<unknown> {
  ...
  if (params.mode === "raw") {
    ...
    const createResult = await client.graphql.mutation<
      CreateReplaySessionRawResult,
      CreateReplaySessionRawVars
    ...
    const session = createResult.createReplaySession.session;
    ...
    return formatSendResult(result, session.id, opts);
  }
