...
export async function caidoSessionsImpl(params: CaidoSessionsParams, client: Client): Promise<unknown> {
  switch (params.action) {
    case "list": {
      const limit = params.limit ?? 50;
      const connection = await client.replay.sessions.list().first(limit);
      return connection.edges.map((e: any) => ({
        id: e.node.id,
        name: e.node.name,
    ...
    case "create": {
      ...
      const session = await client.replay.sessions.create({ requestSource: { id: params.request_id } });
      return { id: session.id, name: session.name, collectionId: session.collectionId };
    }
    case "rename": {
      if (!params.session_id || !params.name) throw new Error("session_id and name are required for rename");
