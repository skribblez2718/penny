...
// ============================================================
// Pi event/context shapes.
// The Pi SDK exposes ExtensionAPI/ExtensionContext/event payloads as `any`
// (see .pi/lib/types/pi-types.d.ts). We describe only the fields this
// extension actually reads so handlers are type-checked against real shapes.
// ============================================================
...
/** Model shape emitted by the `model_select` event (provider always present). */
interface SelectableModel {
  id: string;
  provider: { id: string };
}

/** Subset of the extension context this extension reads. */
...
interface ModelSelectEvent {
  ...
  previousModel?: SelectableModel | null;
  source?: string;
}

/** Context passed to registered slash-command handlers. */
interface CommandContext {
  ui: { notify(message: string, level: string): void };
}

/**
 * Raw Pi conversation message. Fields vary by `role`; all role-specific
 * fields are optional and read defensively in extractMessageData.
 */
...
/** Query parameters shared by the log-query tools. */
...
function connect(force = false): void {
  ...
  try {
    ...
    ws.on("open", () => {
      ...
      // `_socket` is the underlying TCP socket, an internal of the `ws` library
      // with no public typing. Reading it to unref() the event loop handle.
    ...
    ws.on("close", (code, reason) => {
      ...
      if (state.connected) {
        ...
        logger.debug("WebSocket closed during connection attempt", {
          code,
...
function getModelInfo(ctx: EventContext): { provider: string; model: string } | null {
  try {
 ...
 */
function extractMessageData(msg: RawMessage, maxOutputLength: number): Record<string, unknown> {
...
export default async function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (event: SessionStartEvent, ctx: EventContext) => {
  ...
  pi.on("agent_start", async (_event: unknown, ctx: EventContext) => {
    sendMessage("agent_start", {
      model: getModelInfo(ctx),
    });
  });

  pi.on("agent_end", async (event: AgentEndEvent, _ctx: unknown) => {
  ...
  pi.on("message_end", async (event: MessageEndEvent, _ctx: unknown) => {
  ...
  pi.on("tool_execution_start", async (event: ToolExecutionStartEvent, _ctx: unknown) => {
  ...
  pi.on("tool_result", async (event: ToolResultEvent, _ctx: unknown) => {
  ...
  pi.on("model_select", async (event: ModelSelectEvent, _ctx: unknown) => {
  ...
  const restBaseUrl =
    getEnvVar("PI_OBSERVABILITY_REST_URL") ||
  ...
  async function observabilityFetch(path: string): Promise<unknown> {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        if (resp.status === 401 || resp.status === 403) {
          throw new Error(
            `[AUTH] Observability server authentication failed (${resp.status}). Check PI_OBSERVABILITY_API_KEY in .env.`
          );
        }
        throw new Error(
          `[SERVER] Observability server returned ${resp.status}: ${text.slice(0, 200)}`
        );
      }
      return resp.json();
    } catch (err) {
      ...
      if (name === "TimeoutError" || name === "AbortError") {
        throw new Error(
          `[TIMEOUT] Observability server at ${restBaseUrl} did not respond within 10s. Is the server running?`
        );
      }
      if (causeCode === "ECONNREFUSED" || message.includes("fetch failed")) {
        throw new Error(
          `[CONNECTION_REFUSED] Cannot connect to observability server at ${restBaseUrl}. Start it with: python -m observability`
        );
      }
      // Re-throw with context if it's already one of our formatted errors
      if (message.startsWith("[")) throw err;
      throw new Error(`[NETWORK] Observability request failed: ${message || "Unknown error"}`);
    }
  }
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: QueryLogsParams) {
      try {
        ...
        logger.error(
          "observability_query_logs failed",
          { error: msg },
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: QueryHistoryParams) {
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        logger.error(
          "observability_query_history failed",
          { error: msg },
          Object.assign(err instanceof Error ? err : new Error(msg), {
...
function startServer(): boolean {
  ...
  // specific submodules that are needed for server operation. observability is
  // installed editable into .venv via the uv workspace, so no PYTHONPATH hack.
  try {
    execSync(
    ...
    // execSync throws an Error augmented with an untyped `stderr` buffer.
    const stderrVal = (err as { stderr?: unknown }).stderr;
    const stderr = stderrVal != null ? String(stderrVal) : "";
    logger.error(
  ...
  _serverProc.on("exit", (code) => {
    ...
    _restartAttempt++;
    logger.info(
      `Auto-restarting observability server in ${delay / 1000}s (attempt ${_restartAttempt}/${maxRestarts})`
    );
...
function _findPython(projectRoot: string): string {
  // PI_VENV_PYTHON is the optional interpreter override honored by the other Pi
  // extensions (skill, memory, resume, compaction); check it first for parity.
  const candidates = [
  ...
  for (const c of candidates) {
    ...
    // observability is installed editable into .venv via the uv workspace,
    // so no PYTHONPATH is required.
    try {
      execSync(`"${c}" -c "import observability"`, {
        timeout: 5000,
        stdio: "pipe",
      });
      logger.debug("Python env validated", { python: c });
      return c;
    } catch {
      logger.warn(
