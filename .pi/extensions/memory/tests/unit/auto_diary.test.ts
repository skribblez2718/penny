...
describe("observabilityRestFetch", () => {
  ...
  it("returns JSON on successful fetch", async () => {
    ...
    const result = await observabilityRestFetch("http://localhost:8765/sessions/s1/entries", "key123");
    expect(result).toEqual({ items: [], total: 0 });
    expect(fetchSpy).toHaveBeenCalledTimes(1);
    const call = fetchSpy.mock.calls[0] as unknown as [string, { headers: Record<string, string>; signal: AbortSignal }];
...
describe("buildDiaryFromObservability", () => {
  ...
  it("limits tool names to top 6 by count", async () => {
    const fetchSpy = vi.fn((url: string) => {
      ...
      if (url.includes("tool_execution_start")) {
        const items = [
          { data: { toolName: "a" } }, { data: { toolName: "a" } }, { data: { toolName: "a" } },
          { data: { toolName: "b" } }, { data: { toolName: "b" } },
          { data: { toolName: "c" } }, { data: { toolName: "c" } },
          { data: { toolName: "d" } },
          { data: { toolName: "e" } },
          { data: { toolName: "f" } },
          { data: { toolName: "g" } },
