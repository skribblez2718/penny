 ...
 *
 * Verifies that the fix for the "spawn <venv>/bin/python ENOENT" error
 * (Node 20+ masking missing-cwd errors as ENOENT on the spawn
...
// Resolve the REAL venv python dynamically instead of hardcoding a
// machine-specific path. Prefer PI_VENV_PYTHON (the same source of truth the
// skill extension itself checks first); fall back to the repo-relative .venv
// when the env var is unset (e.g. a fresh checkout / CI). The test file lives
// at .pi/extensions/skill/tests/unit/, so the project root is five levels up.
const __testDir = dirname(fileURLToPath(import.meta.url));
const PROJECT_ROOT = join(__testDir, "..", "..", "..", "..", "..");
const VENV_PYTHON = process.env.PI_VENV_PYTHON || join(PROJECT_ROOT, ".venv", "bin", "python");
const HAS_VENV = existsSync(VENV_PYTHON);
...
describe("callPython: cwd ENOENT safety", () => {
  ...
  it.skipIf(!HAS_VENV)(
    "the Node 20+ spawn ENOENT can be reproduced with a non-existent cwd",
    () => {
      ...
      return new Promise<void>((resolve) => {
        const proc = spawn(VENV_PYTHON, ["-c", "print('hi')"], {
  ...
  it.skipIf(!HAS_VENV)(
    "the Node 20+ spawn ENOENT is avoided after mkdirSync({ recursive: true })",
    () => {
      const { spawn } = require("child_process");
      const { mkdirSync } = require("fs");
      const fakeCwd = join(freshDir(), "does-not-exist");
  ...
  it.skipIf(!HAS_VENV)("realpathSync resolves a working venv-python symlink chain", () => {
    // Verifies the extension-load-time resolver can find the real
    // executable behind the venv -> base-interpreter symlink chain (used for
    // the exec-bit check; the resolver SPAWNS the symlink path, not the real).
    const { realpathSync, statSync } = require("fs");
    const candidate = VENV_PYTHON;
