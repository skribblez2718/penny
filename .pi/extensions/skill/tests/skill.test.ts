...
import { parseSummaryFromOutput, defaultSummaryForAgent, formatResult } from "../skill-utils.js";
...
describe("defaultSummaryForAgent", () => {
  it("should return explore default with explore_complete FALSE (safe)", () => {
    const result = defaultSummaryForAgent("echo");
    expect(result.explore_complete).toBe(false);
    expect(result.findings_count).toBe(0);
    expect(result.files_count).toBe(0);
    expect(result.unknowns_count).toBe(0);
  });

  it("should return plan default with plan_complete FALSE (safe)", () => {
    const result = defaultSummaryForAgent("piper");
    expect(result.plan_complete).toBe(false);
    expect(Array.isArray(result.plan_steps)).toBe(true);
    expect((result.plan_steps as any[]).length).toBe(0);
  });

  it("should return critique default with NEEDS_REVISION (safe)", () => {
    const result = defaultSummaryForAgent("carren");
    expect(result.verdict).toBe("NEEDS_REVISION");
    expect(Array.isArray(result.issues)).toBe(true);
    expect((result.issues as string[]).length).toBe(1);
    expect((result.issues as string[])[0]).toContain("SUMMARY");
  });

  it("should return taskifier default with complete FALSE (safe)", () => {
    const result = defaultSummaryForAgent("tabitha");
  ...
  it("should return empty object for unknown agent", () => {
    const result = defaultSummaryForAgent("unknown");
    expect(Object.keys(result)).toHaveLength(0);
  });

  it("should NOT claim explore_complete=true for echo (old bug)", () => {
    // Regression test: the old default claimed explore_complete=true,
    // which caused the skill to advance on empty agent output.
    const result = defaultSummaryForAgent("echo");
    // The safe default must be FALSE
    expect(result.explore_complete).not.toBe(true);
  });

  it("should NOT claim complete=true for tabitha (old bug)", () => {
    // Regression test: the old default sometimes claimed complete=true
    const result = defaultSummaryForAgent("tabitha");
    expect(result.complete).not.toBe(true);
  });

  it("should return prd-canonical synthia default (orchestrator contract)", () => {
    // The prd skill's orchestrator (the source of truth for its own
    // schema) expects a specific shape. When the agent's stdout has
    // no parseable SUMMARY block, this fallback must match exactly
    // so validation passes and the orchestrator can proceed. The
    // synthia case was the source of an infinite-loop bug in
    // June 2026: the old fallback returned
    // { synthesis_complete: false, theme_count: 0, source_count: 0 }
    // which did not match the orchestrator's expected prd fields
    // and caused repeated "missing required fields" rejections.
    const result = defaultSummaryForAgent("synthia");
    // Must match .pi/skills/prd/scripts/orchestrate.py _safe_default_summary()
    expect(result.requirement_count).toBe(0);
    expect(result.narrative_sections).toBe(0);
    expect(result.verification_matrix_complete).toBe(false);
    expect(result.ideal_state_valid).toBe(false);
    expect(result.needs_clarification).toBe(false);
    expect(Array.isArray(result.clarifying_questions)).toBe(true);
    expect((result.clarifying_questions as any[]).length).toBe(0);
    expect(result.complete).toBe(false);
    // Negative assertion: must NOT have the old non-canonical fields
    expect(result).not.toHaveProperty("synthesis_complete");
    expect(result).not.toHaveProperty("theme_count");
    expect(result).not.toHaveProperty("source_count");
  });

  it("should return prd-canonical vera default (orchestrator contract)", () => {
    // Same orchestrator-authority principle as synthia: the fallback
    // must match the prd orchestrator's expected vera fields exactly.
    const result = defaultSummaryForAgent("vera");
    ...
    expect(result).not.toHaveProperty("validation_complete");
    expect(result).not.toHaveProperty("quality_gate_passed");
  });
});
