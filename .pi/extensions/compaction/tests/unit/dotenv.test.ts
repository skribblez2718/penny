...
describe("compaction .env fallback loader", () => {
  ...
  it("reads simple KEY=VALUE pairs from .env file", () => {
    writeFileSync(join(TMP_DIR, ".env"), [
  ...
  it("prefers process.env over .env file values", () => {
    ...
    expect(getEnvVar("TEST_PREFERS_ENV", { TEST_PREFERS_ENV: "from_dotenv" })).toBe("from_process_env");
  ...
  it("falls back to .env when process.env is empty string", () => {
    ...
    expect(getEnvVar("TEST_FALLBACK_KEY", { TEST_FALLBACK_KEY: "from_dotenv" })).toBe("from_dotenv");
