...
import { fetchTranscript, extractVideoId, TranscriptResult } from "./client.js";
import {
...
const SUPPORTED_LANGUAGES = [
  ...
  "es", "fr", "de", "it", "pt", "nl", "ko", "ja", "zh", "ru",
  "ar", "hi", "th", "vi", "id", "ms", "tr", "pl", "sv", "da", "no", "fi",
];
...
export default async function youtubeExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      url: Type.String({
        description: "YouTube video URL or video ID. Examples: https://youtube.com/watch?v=VIDEO_ID, https://youtu.be/VIDEO_ID, VIDEO_ID",
      }),
      format: Type.Optional(
        Type.Union(
          FORMAT_OPTIONS.map(f => Type.Literal(f)),
          { description: "Output format: text (plain transcript), json (structured with timing), srt, or webvtt" }
        )
      ),
    ...
    async execute(_toolCallId: string, params: any) {
      try {
        ...
        if (err instanceof YoutubeTranscriptVideoUnavailableErrorClass) {
          userMessage = "Video not found or unavailable. Check the URL and verify the video is public.";
        } else if (err instanceof YoutubeTranscriptNotAvailableLanguageErrorClass) {
          userMessage = "No captions available for this video. Use languages=['auto'] to try auto-generated captions.";
  ...
  pi.registerCommand("youtube-status", {
    ...
    handler: async (_args: string, ctx: any) => {
      try {
        ...
        ctx.ui.notify(
          `YouTube Extension: ✅ Ready (tested video ID: ${testVideoId})`,
          "info"
        );
      } catch (err) {
        ctx.ui.notify(
