...
export function createQuestionnaireUI(
  ...
  editor.onSubmit = (value: string) => {
    ...
    if (q && q.type === "multi" && multiSelections.has(inputQuestionId)) {
...
export default function questionnaire(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    async execute(
      ...
      const questions: Question[] = params.questions.map((q: Question, i: number) => ({
        ...q,
        label: q.label || `Q${i + 1}`,
      ...
      if (!ctx.hasUI) {
        ...
        for (const q of questions) {
          ...
          answers.push({
            ...
            label: q.type === "multi" ? "(multi-select)" : "Waiting for user input (non-interactive mode)",
            wasCustom: false,
          });
        }
      ...
      // @ts-expect-error - Pi infers generic at runtime
    ...
    renderResult(result: any, _options: any, theme: any) {
      const details = result.details as
        | (QuestionnaireResult & { needsUserInput?: boolean })
        | undefined;
      if (!details) {
