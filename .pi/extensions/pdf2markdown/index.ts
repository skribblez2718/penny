...
// @opendocsg/pdf2md is a CommonJS module (`module.exports = fn`). Depending on the
// ESM interop (native Node vs. jiti), the default import is either the function
// itself or a namespace object wrapping it in `.default` — handle both while keeping
// the package's real call signature.
import pdf2mdModule from "@opendocsg/pdf2md";
const pdf2md = (pdf2mdModule as { default?: typeof pdf2mdModule }).default ?? pdf2mdModule;
...
export default function pdf2markdownExtension(pi: ExtensionAPI) {
  ...
  pi.on(
    "session_start",
    async (_event: unknown, ctx: { sessionManager: { getSessionId: () => string } }) => {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      pdfPath: Type.String({
        description:
          "Path to the PDF file to convert. Relative paths are resolved from the current working directory.",
      }),
