...
// njsscan binary is on this host (Batch F.2). The not-installed path stays
...
describe("njsscan extension", () => {
  ...
  it("registers exactly one njsscan_scan tool", async () => {
    await extension(mockPi);
    expect(registeredTools).toHaveLength(1);
    expect(registeredTools[0].name).toBe("njsscan_scan");
  });
  ...
  it("returns a graceful not-installed message with the pinned version (forced)", async () => {
    ...
    await extension(mockPi);
    const tool = registeredTools.find((t) => t.name === "njsscan_scan")!;
    const res = await tool.execute("test-id", { target: "/tmp/does-not-exist" });
    const text = res.content[0].text;
    expect(text).toContain("v0.4.3");
    expect(text.toLowerCase()).toContain("not installed");
  });

  it("is presence-aware: on this host (njsscan present) it runs instead of returning the not-installed stub", async () => {
    const dir = mkdtempSync(join(tmpdir(), "njsscan-pa-"));
    writeFileSync(join(dir, "app.js"), 'console.log("hi");\n');
    await extension(mockPi);
    const tool = registeredTools.find((t) => t.name === "njsscan_scan")!;
    const res = await tool.execute("test-id", { target: dir });
    const parsed = JSON.parse(res.content[0].text);
    expect(parsed.tool).toBe("njsscan");
    expect(typeof parsed.success).toBe("boolean");
    if (parsed.installed === false) {
      expect(res.content[0].text).toContain("v0.4.3");
