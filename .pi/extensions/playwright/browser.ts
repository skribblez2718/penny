...
export class BrowserManager {
  ...
  /**
   * Get the underlying Playwright Browser instance (or null if not launched).
   * Exposed for tools that need raw browser access (e.g. run_code_unsafe).
   */
  getRawBrowser(): Browser | null {
    return this.browser;
  }
  ...
  async snapshot(): Promise<{ tree: SnapshotNode | null; error?: string }> {
    ...
    try {
      const tree = (await page.evaluate(() => {
        ...
        function getState(el: Element): Record<string, unknown> | undefined {
