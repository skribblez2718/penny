...
export function registerPdfTools(pi: ExtensionAPI, config: PlaywrightConfig) {
  ...
  pi.registerTool({
    ...
    label: "Save as PDF",
    description:
      "Save the current page as a PDF file. Only works in Chromium headless mode.",
    ...
    async execute(_toolCallId, params) {
      ...
      const outputDir = params.path
        ? dirname(params.path as string)
        : config.outputDir;
      if (!existsSync(outputDir)) mkdirSync(outputDir, { recursive: true });

      const path =
        (params.path as string) || `${outputDir}/page-${Date.now()}.pdf`;
