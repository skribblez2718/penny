...
export function registerRouteFormFileTools(pi: ExtensionAPI, _config: PlaywrightConfig) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      action: Type.Union(
        [Type.Literal("abort"), Type.Literal("fulfill")],
        { description: "Action to take for matching requests" }
      ),
      body: Type.Optional(
        Type.String({ description: "Response body (for fulfill)" })
      ),
