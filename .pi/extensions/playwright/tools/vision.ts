...
export function registerVisionDevtoolsTools(pi: ExtensionAPI, _config: PlaywrightConfig) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use for testing drag-based interactions at the coordinate level.",
    ],
    parameters: Type.Object({
  ...
  pi.registerTool({
    ...
    description:
      "Highlight an element on the page with a visible overlay (useful for debugging).",
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use playwright_hide_highlight to clean up visual debugging markers.",
    ],
    parameters: Type.Object({}),
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      const page = await browser.getPage();
      const path =
        (params.path as string) || `/tmp/playwright-output/trace-${Date.now()}.zip`;
