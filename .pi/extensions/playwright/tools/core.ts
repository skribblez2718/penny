...
export function registerCoreTools(pi: ExtensionAPI, _config: PlaywrightConfig) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      path: Type.Optional(Type.String({ description: "File path to save the screenshot to" })),
    }),
    async execute(_toolCallId, params) {
...
function truncateTree(node: SnapshotNode, maxNodes: number): SnapshotNode | null {
  ...
  function walk(n: SnapshotNode): SnapshotNode | null {
    ...
    remaining--;
    const children = n.children?.map(walk).filter((c): c is SnapshotNode => c !== null);
