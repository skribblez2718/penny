...
| validating | vera | anti-fabrication trace + compliance (bounded revise loop) — **evidence-gated** (Rec 4): `REZ_VALIDATE` requires a non-empty `evidence` field (captured per-bullet source traceability + STAR/ATS/NICE checks), so a resume is never marked valid on a bare assertion. The fresh-NICE lookup (`aligning`, live retrieval every run) is engine-standard leverage. |
