...
def warn_joern_unavailable() -> str:
    ...
    Every branch points at the canonical installer
    (``scripts/setup/init-joern.sh``, also run by ``make setup``) so the
    message is actionable regardless of why Joern is unavailable.
    """
    # Canonical install path — building the jsa-joern Docker image.
    install_hint = (
        "To install, run `bash scripts/setup/init-joern.sh` (also run by "
        "`make setup`), which builds the Joern Docker image."
    )
    if not _can_run_docker():
        return (
            ...
            "Install Docker, then " + install_hint
        )
    if not _has_joern_image():
        return (
            f"Joern Docker image '{DOCKER_IMAGE}' not found. "
            "SLICE phase will operate in degraded mode. " + install_hint
        )
    return (
