...
Architecture: plans/jsa-implementation/03-splitter-and-dedup-design.md §A.7
"""
...
def split_js_multi(
    files: list[tuple[str, str]],          # [(path, source_code), ...]
    max_tokens_per_chunk: int = 12000,
    overlap_tokens: int = 2000,
    active_analyzers: list[str] | None = None,
) -> MultiSplitResult:
    """
    Concatenate all JS files into one stream, split into chunks,
    resolve byte offsets to original file locations.
    
    This is the main entry point for the jsa skill's ACQUIRE phase.
    """
    
    # Step 1: Concatenate with delimiters
    concatenated = ""
    entries = []
    current_byte = 0
    ...
    for file_path, source in files:
        delimiter = f"\n// === file: {file_path} ===\n"
        section = delimiter + source
        start = current_byte
        end = start + len(section)
        
        entries.append(FileMapEntry(
            file_path=file_path,
            concat_start_byte=start,
            concat_end_byte=end,
            original_line_count=source.count('\n') + 1,
        ))
        
        concatenated += section
        current_byte = end
    
    file_map = FileMap(entries=entries)
    
    # Step 2: Split the concatenated stream
    split_result = split_js(
        concatenated,
        max_tokens=max_tokens_per_chunk,
        overlap_tokens=overlap_tokens,
        file_path="__concatenated__",
    )
    
    # Step 3: Resolve each chunk to file locations
    resolved = []
    for chunk in split_result.chunks:
        file_spans = _resolve_byte_range(chunk.start_byte, chunk.end_byte, file_map, files)
        resolved.append(ResolvedChunk(
            ...
            metadata={
                **chunk.metadata,
                ...
                "active_analyzers": active_analyzers or [],
            }
        ))
    ...
    return MultiSplitResult(
        ...
        total_tokens=split_result.total_tokens,
...
if __name__ == "__main__":
    ...
    small = "const x = 1;\nfunction hello() {\n  console.log('hi');\n}\n"
    result = split_js(small)
    assert result.method == "single_chunk"
    assert result.chunk_count == 1
    print(f"Test 1 OK: small file → {result.method}, {result.chunk_count} chunk")
    
    # Test 2: Multi-file concatenation
    files = [
        ("app.js", "const API = '/api';\nfunction init() {\n  fetch(API);\n}\n"),
        ("utils.js", "function format(d) {\n  return d.trim();\n}\n"),
    ]
    multi = split_js_multi(files)
    print(f"Test 2 OK: {len(files)} files → {multi.chunk_count} chunks, method={multi.method}")
    for c in multi.chunks:
        print(f"  {c.chunk_id}: {len(c.file_spans)} file_spans")
    
    # Test 3: Token estimation
    assert estimate_tokens("hello world") > 0
    print("Test 3 OK: estimate_tokens works")
