"""jsa Skill - Pipeline FSM

17-phase state machine orchestrating the jsa pipeline.
Uses python-statemachine for phase transitions.

Phases:
  INTAKE → ACQUIRE → CVE_RESEARCH → SAST_SCAN → NORMALIZE → DEDUP_WITHIN_SOURCE → CORRELATE_EVIDENCE → AGENT_REVIEW → SAST_VALIDATE → CHUNK → DISPATCH → COLLECT → MERGE → VERIFY → REPORT → REFLECT → COMPLETED
...
class JSAPhase(Enum):
    # INTAKE is a configuration phase, not a runtime phase. As of 2026-06
    # it no longer invokes a separate questionnaire tool — instead, the user
    # provides target configuration directly via skill(goal, constraints).
    # If required fields are missing, INTAKE returns a `complete` action
    # describing what's missing and Penny uses the questionnaire tool to
    # collect it. Once intake is complete, INTAKE advances to ACQUIRE.
    INTAKE = auto()
    ACQUIRE = auto()
    CVE_RESEARCH = auto()
    SAST_SCAN = auto()
    NORMALIZE = auto()
    DEDUP_WITHIN_SOURCE = auto()
    CORRELATE_EVIDENCE = auto()
    AGENT_REVIEW = auto()  # P10: agent reviewer on bounded evidence packets
    SAST_VALIDATE = auto()
    # STOP is an optional interactive checkpoint, not a permanent pipeline phase.
    # It is included in the enum so the FSM can route through it when active,
    # but it is not listed in the canonical phase flow (see module docstring).
    STOP = auto()
    # CHUNK has been REMOVED as of 2026-06 (Phase B of structure-and-slice migration).
    # AST-aware chunking was the anti-pattern the document warned against.
    # Replaced by STRUCTURE (build typed store + cards) + SLICE (per-class slicing).
    # STRUCTURE = auto()  # see below — was previously between STOP and DISPATCH
    STRUCTURE = auto()
    SLICE = auto()
    # DISPATCH has been RENAMED to INVESTIGATE as of 2026-06 (Phase B).
    # It now consumes FlowCards (from SLICE) rather than raw chunks.
    # INVESTIGATE runs wave-based: Python verification + N sequential annie
    # subagent dispatches, tracked via state.metadata["investigate_wave"].
    INVESTIGATE = auto()
    COLLECT = auto()
    MERGE = auto()
    VERIFY = auto()
...
class JSAState:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "module_cards": [mc.to_dict() for mc in self.module_cards],
            "page_cards": [pc.to_dict() for pc in self.page_cards],
            "flow_cards": [fc.to_dict() for fc in self.flow_cards],
...
class JSAPhaseMachine(StateMachine):
    """FSM for jsa pipeline using python-statemachine v3."""

    _ = States.from_enum(
        JSAPhase,
        initial=JSAPhase.INTAKE,
        final=JSAPhase.COMPLETED,
    )

    # Transitions
    # INTAKE has no outgoing FSM event — it's terminal-as-config-phase.
    # _build_action() detects "intake complete" and explicitly advances
    # the FSM to ACQUIRE by calling fsm.advance(JSAPhase.ACQUIRE) directly.
    go_acquire = _.INTAKE.to(_.ACQUIRE)
    go_cve_research = _.ACQUIRE.to(_.CVE_RESEARCH)
    go_sast_scan = _.CVE_RESEARCH.to(_.SAST_SCAN)
    go_normalize = _.SAST_SCAN.to(_.NORMALIZE)
    go_dedup_within = _.NORMALIZE.to(_.DEDUP_WITHIN_SOURCE)
    go_correlate = _.DEDUP_WITHIN_SOURCE.to(_.CORRELATE_EVIDENCE)
    go_agent_review = _.CORRELATE_EVIDENCE.to(_.AGENT_REVIEW)
    go_sast_validate = _.AGENT_REVIEW.to(_.SAST_VALIDATE)
    go_structure = _.SAST_VALIDATE.to(_.STRUCTURE)
    go_slice = _.STRUCTURE.to(_.SLICE)
    go_investigate = _.SLICE.to(_.INVESTIGATE)
    go_stop = _.INVESTIGATE.to(_.STOP)
    go_collect = _.STOP.to(_.COLLECT)
    go_merge = _.COLLECT.to(_.MERGE)
    go_verify = _.MERGE.to(_.VERIFY)
    go_report = _.VERIFY.to(_.REPORT)
    go_reflect = _.REPORT.to(_.REFLECT)
    go_completed = _.REFLECT.to(_.COMPLETED)

    # ANALYZE is handled by annie agents during DISPATCH - not a separate phase

    _event_map = {
        JSAPhase.ACQUIRE: "go_acquire",
        JSAPhase.CVE_RESEARCH: "go_cve_research",
        JSAPhase.SAST_SCAN: "go_sast_scan",
        JSAPhase.NORMALIZE: "go_normalize",
        JSAPhase.DEDUP_WITHIN_SOURCE: "go_dedup_within",
        JSAPhase.CORRELATE_EVIDENCE: "go_correlate",
        JSAPhase.AGENT_REVIEW: "go_agent_review",
        JSAPhase.SAST_VALIDATE: "go_sast_validate",
        JSAPhase.STOP: "go_stop",
        JSAPhase.STRUCTURE: "go_structure",
        JSAPhase.SLICE: "go_slice",
        JSAPhase.INVESTIGATE: "go_investigate",
        JSAPhase.COLLECT: "go_collect",
        JSAPhase.MERGE: "go_merge",
        JSAPhase.VERIFY: "go_verify",
        JSAPhase.REPORT: "go_report",
        JSAPhase.REFLECT: "go_reflect",
        JSAPhase.COMPLETED: "go_completed",
    }

    def __init__(self, start_phase: JSAPhase = JSAPhase.INTAKE):
        super().__init__(start_value=start_phase)
        self._history: list[str] = [self.current_state_value.name]

    def after_transition(self, event: str, source, target):
        self._history.append(target.value.name)

    @property
    def phase(self) -> JSAPhase:
    ...
    def advance(self, target: JSAPhase) -> bool:
        event_name = self._event_map.get(target)
...
# ---------------------------------------------------------------------------
# 1.3.4 Intake handler
# ---------------------------------------------------------------------------

def intake_handler(state: JSAState, params: dict) -> JSAState:
    """Parse skill parameters and initialize state."""
    goal = params.get("goal", "")
    state.target_url = goal
    state.analyzers = params.get("analyzers", [])

    # Extract target URL from goal
    for word in goal.split():
        if word.startswith("http://") or word.startswith("https://"):
            state.target_url = word
            break

    # Set output directory: user-specified > default /tmp/jsa-{hostname}
    if params.get("output_dir"):
        state.output_dir = params["output_dir"]
    else:
        hostname = "unknown"
        try:
            from urllib.parse import urlparse
            parsed = urlparse(state.target_url)
            hostname = parsed.netloc.split(":")[0].replace(".", "-") or "unknown"
        except Exception:
            pass
        state.output_dir = f"/tmp/jsa-{hostname}"

    # Ensure output directories exist
    state.ensure_dirs()

    if not state.analyzers:
        state.analyzers = _get_all_analyzers()

    state.metadata["intake_completed"] = True
    state.metadata["output_structure"] = {
        "session": str(Path(state.output_dir) / "session.json"),
        "report": str(Path(state.output_dir) / "report.md"),
        "assets": str(state.assets_dir),
        "sast": str(state.sast_dir),
        "findings": str(state.findings_dir),
        "evidence": str(state.evidence_dir),
    }
    state.updated_at = datetime.now(timezone.utc).isoformat()
    return state


def _get_all_analyzers() -> list[str]:
    """List all available vulnerability analyzers."""
    # In production, discover from scripts/analyzers/ directory
    return [
        "dom_xss", "reflected_xss", "stored_xss",
        "prototype_pollution", "csti", "postmessage",
        "dom_clobbering", "open_redirect", "ssrf", "sqli",
        "secret_disclosure", "request_override", "link_manipulation",
        "http_header_injection", "dom_data_manipulation",
        "csrf", "cors", "clickjacking", "idor",
        "http_smuggling", "cache_poisoning", "insecure_deserialization",
    ]


# ---------------------------------------------------------------------------
# 1.3.5 Acquire handler
# ---------------------------------------------------------------------------

def acquire_handler(state: JSAState) -> JSAState:
    """
    Download JS files, pre-filter, concatenate, and chunk.

    In production, this calls the echo agent via subagent().
    For unit testing, this can be called directly with stubbed data.
    """
    # In real pipeline, echo agent produces (files, file_map, chunks)
    # Here we record the phase transition and expectations

    state.metadata["acquire_started"] = True
    state.metadata["acquire_expected"] = {
        "target": state.target_url,
        "analyzers": state.analyzers,
        "pipeline": "echo → pre-filter → split_js_multi",
    }
    state.updated_at = datetime.now(timezone.utc).isoformat()
...
def _do_acquire(state: JSAState, js_files: list[tuple[str, str]]) -> JSAState:
    """
    Execute the full acquisition pipeline (called by echo agent result handler).

    Phase E (2026-06): Updated to use the new structure-and-slice architecture.
    Instead of chunking the concatenated JS corpus, we now build a typed
    analysis store via STRUCTURE and emit candidate flows via SLICE.

    Args:
        js_files: List of (file_path, source_code) tuples
    """
    from structure_analysis import build_file_manifest, build_ast_index

    # Build file manifest and AST indexes (deterministic, local)
    manifest = build_file_manifest(js_files)
    ast_indexes = build_ast_index(js_files)  # expects list[tuple[str, str]]

    state.file_map = manifest
    state.typed_store = {
        "manifest": manifest,
...
def sast_scan_handler(state: JSAState, js_files: list[tuple[str, str]] | None = None) -> JSAState:
    """
    Run semgrep + jsluice on all downloaded JS files.

    This is a FAST pre-scan before per-class slicing. Findings are stored
    in the mesh for the SAST_VALIDATE phase and later for vuln class
    agents to reference.

    In production, this runs semgrep_scan + jsluice_secrets tools.
    For testing, findings can be injected directly.
    """
    state.metadata["sast_scan"] = {
        "status": "planned",
        "tools": ["semgrep_scan", "jsluice_secrets", "jsluice_urls"],
        "target": "All downloaded JS files (pre-chunking)",
    }
    state.updated_at = datetime.now(timezone.utc).isoformat()
    return state
...
def dedup_handler(state: JSAState) -> None:
    """
    Backward-compatible wrapper for the three-phase DEDUP pipeline.

    When called directly (legacy code, tests), runs all three sub-phases:
    1. normalize_handler() — component normalization + vulnerability canonicalization
    2. dedup_within_source_handler() — SAST fingerprint dedup
    3. correlate_evidence_handler() — cross-stream correlation
    """
...
def agent_reviewer_handler(state: JSAState) -> None:
    ...
    CORRELATE_EVIDENCE phase), then produces a local heuristic review.
    In production, each packet is dispatched to annie agent with the
    annie-agent-review.md prompt for structured judgment.
...
def _local_agent_review(packets) -> list[dict]:
    """
    Local heuristic agent review for testing. Produces verdicts based on
    deterministic signal combination, mimicking what the agent would produce.

    In production, each packet would be dispatched to annie with
    annie-agent-review.md prompt and real judgment.
    """
    verdicts = []
...
def sast_validate_handler(state: JSAState) -> JSAState:
    ...
    In production, calls annie agent with annie-sast-validate.md prompt.
    The agent reviews each SAST finding and classifies it.

    Validated findings are stored in sast_validated and made available to
    vuln class agents during DISPATCH so they know what to skip.
    """
    state.metadata["sast_validate"] = {
        "status": "planned",
        "agent": "annie",
        "prompt": "assets/prompts/annie-sast-validate.md",
...
def chunk_handler(state: JSAState, js_files: list[tuple[str, str]] | None = None) -> JSAState:
    """
    DEPRECATED (Phase B, 2026-06): CHUNK phase removed.

    AST-aware chunking of concatenated JS was the anti-pattern the
    structure-and-slice document warned against. Replaced by STRUCTURE
    (build typed store) + SLICE (per-class candidate generation).

    This function is kept as a no-op stub for backward compatibility
    with any code that imports it. New code should use structure_handler
    and slice_handler instead.
    """
    state.metadata["chunk_deprecated"] = True
    return state
...
def structure_handler(state: JSAState, js_files: list[tuple[str, str]] | None = None) -> JSAState:
    ...
    Phase C will implement the full builder. For now, this is a stub
    that records the phase ran and creates empty card collections.
...
def slice_handler(state: JSAState) -> JSAState:
    ...
    Phase C will implement the full candidate generation. For now, this
    is a stub that records the phase ran and creates empty card collections.
    ...
    for vuln_class, candidates in sast_candidates.items():
        ...
        for cand in candidates[:20]:
            ...
            flow_card = FlowCard(
                ...
                sanitizer_chain=[],
                module_card_ids=[mc.filename for mc in state.module_cards
                                if mc.filename == cand.get("file")],
                page_card_ids=[],
                evidence=[],
                runtime_evidence=[],
...
def _infer_vuln_class_from_rule(rule_id: str, finding: dict) -> Optional[str]:
    ...
    if re.search(r"\bcsrf\b", rid):
        return "csrf_dom"  # Default to DOM; INVESTIGATE may reclassify
...
def _infer_sink_for_vuln(vuln_class: str) -> str:
    ...
    sink_map = {
        ...
        "csrf_dom": "form.submit",
        "csrf_network": "request_without_token",
...
def investigate_handler(state: JSAState) -> JSAState:
    ...
    for pc in page_cards:
        ...
        network_analyzers = ["cors", "clickjacking", "idor",
                              "cache_poisoning", "http_smuggling", "csrf_network"]
...
dispatch_handler = investigate_handler
...
def merge_handler(state: JSAState) -> JSAState:
    """Run merge and dedup on collected findings."""
    from dedup import Finding, merge_and_dedup

    if not state.raw_findings:
        state.metadata["merge_result"] = {"total_raw": 0, "total_merged": 0}
        return state
...
# ---------------------------------------------------------------------------
# 1.3.9 Verify handler
# ---------------------------------------------------------------------------

def verify_handler(state: JSAState) -> JSAState:
    """
    Browser PoC verification of merged findings.

    In production, calls vera agent per finding.
    For unit testing, records verification plan.
    """
    state.metadata["verify_plan"] = {
        "findings_to_verify": len(state.merged_findings),
        "agent": "vera",
        "protocol": "assets/prompts/vera-base.md",
    }
    state.updated_at = datetime.now(timezone.utc).isoformat()
...
# ---------------------------------------------------------------------------
# 1.3.10 Report handler
# ---------------------------------------------------------------------------

def report_handler(state: JSAState) -> JSAState:
    """
    Generate structured finding report.

    In production, calls skribble agent.
    For unit testing, records report plan.
    """
    state.metadata["report_plan"] = {
        "findings_to_report": len(state.merged_findings),
        "agent": "skribble",
...
# ---------------------------------------------------------------------------
# 1.3.11 Reflect handler
# ---------------------------------------------------------------------------

def reflect_handler(state: JSAState) -> JSAState:
    """
    Cross-session pattern learning.

    In production, calls carren + synthia agents.
    For unit testing, records reflection plan.
    """
    state.metadata["reflect_plan"] = {
        "agents": ["carren", "synthia"],
        "learnings_room": "jsa-learnings",
...
# ---------------------------------------------------------------------------
# 1.3.12 Orchestrator entry point
# ---------------------------------------------------------------------------
...
def run_pipeline(
    ...
    """
    Run the full jsa pipeline.

    For production, target_url triggers echo agent → full pipeline.
    For testing, js_files bypasses echo (direct acquire).

    Args:
        target_url: Target URL to analyze
        analyzers: List of vuln classes (default: all)
        output_dir: Directory for reports
        session_id: Resume existing session (optional)
        js_files: Direct file input for testing [(path, source), ...]
        max_tokens_per_chunk: Splitter config
        overlap_tokens: Splitter config
    """
    # State
    state = JSAState(
        session_id=session_id or str(uuid.uuid4())[:12],
        target_url=target_url,
        output_dir=output_dir or f"/tmp/jsa-{_hostname_from_url(target_url)}",
    ...
    # FSM
    fsm = JSAPhaseMachine(start_phase=JSAPhase.INTAKE)

    # Phase handlers — start with ACQUIRE (STOP comes after)
    _run_phase(state, fsm, JSAPhase.ACQUIRE,
               lambda s: _do_acquire(s, js_files) if js_files else acquire_handler(s))

    if js_files:
        _do_acquire(state, js_files)
    else:
        acquire_handler(state)
    ...
    sast_scan_handler(state, js_files if js_files else None)

    # Priority 9: Three-phase DEDUP restructure
    ...
    sast_validate_handler(state)

    fsm.advance(JSAPhase.STRUCTURE)
    structure_handler(state, js_files if js_files else None)

    fsm.advance(JSAPhase.SLICE)
    slice_handler(state)

    fsm.advance(JSAPhase.INVESTIGATE)
    investigate_handler(state)
    ...
    fsm.advance(JSAPhase.MERGE)
    merge_handler(state)

    fsm.advance(JSAPhase.VERIFY)
    verify_handler(state)

    fsm.advance(JSAPhase.REPORT)
    report_handler(state)

    fsm.advance(JSAPhase.REFLECT)
    reflect_handler(state)

    fsm.advance(JSAPhase.COMPLETED)

    state.metadata["phase_history"] = fsm.history
    state.metadata["final_phase"] = fsm.phase.name
...
def _save_session(state: JSAState):
    """Persist session state to session.json."""
    state.ensure_dirs()
    session_path = Path(state.output_dir) / "session.json"
    session_path.write_text(json.dumps(state.to_dict(), indent=2, default=str))
...
def _hostname_from_url(url: str) -> str:
    try:
        from urllib.parse import urlparse
        parsed = urlparse(url)
        return parsed.netloc.split(":")[0].replace(".", "-") or "unknown"
    except Exception:
        return "unknown"


def _run_phase(state: JSAState, fsm: JSAPhaseMachine, phase: JSAPhase, handler, *args):
    """Execute a phase handler and advance FSM."""
    fsm.advance(phase)
...
if __name__ == "__main__":
    # Test: basic pipeline with dummy files
    files = [
    ...
    print(f"Analyzers: {state.analyzers}")
    # Phase E: chunks removed; show module/page/flow card counts
    print(f"Module cards: {len(state.module_cards)}")
    print(f"Page cards: {len(state.page_cards)}")
    print(f"Flow cards: {len(state.flow_cards)}")
    print(f"Merge: {state.metadata.get('merge_result', {})}")
    print(f"Phase history: {state.metadata.get('phase_history', [])}")
    print(f"Final phase: {state.metadata.get('final_phase', 'unknown')}")
