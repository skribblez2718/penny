...
Resolve only the exact closed strategy-blocking evidence gap admitted by the host after Piper orientation or a reviewer identifies it. This is a bounded evidence-acquisition step, not open-ended research.
...
- Start from the admitted `PlanRequestV1`, optional exact `GroundedSynthesisV1` or `DecisionV2`, exact Piper orientation, and exact reviewer repair context. When caller constraints permit, acquire only narrowly targeted read-only local or web evidence for the one admitted gap.
...
- Do not use memory as an evidence source, invent evidence, mutate anything, execute application business logic, design or author the strategy, create tasks, or approve action.
