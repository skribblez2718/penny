...
Normal completion (no clarification needed):
...
When you need clarification (critical information missing):

```
SUMMARY:{"findings_count":<number>,"files_count":<number>,"unknowns_count":<number>,"explore_complete":false,"mempalace_drawer":"<drawer_id from memory_add_drawer>","needs_clarification":true,"clarifying_questions":["<question 1>","<question 2>"],"confidence":"UNCERTAIN"}
```
...
- `confidence` is REQUIRED when `needs_clarification` is `true` — you MUST set `"confidence":"UNCERTAIN"`. This is an explore branch that fans in with its siblings; the orchestrator aggregates branch confidence and only pauses the whole run to ask the user when a branch reports `UNCERTAIN`. If you flag `needs_clarification` without `UNCERTAIN` confidence, your questions are silently dropped and planning proceeds on incomplete context. When `needs_clarification` is `false`, omit `confidence` (or set it to your honest level: `CERTAIN` / `PROBABLE` / `POSSIBLE`).

**Rules:** Single-line valid JSON prefixed with `SUMMARY:` (no space before brace). `findings_count`, `files_count`, `unknowns_count` are integers; `explore_complete` is boolean; `mempalace_drawer` is the drawer ID; `confidence` (when present) is one of `CERTAIN` / `PROBABLE` / `POSSIBLE` / `UNCERTAIN`. Escape quotes with `\"`.
