...
**Critique is evidence-gated** (Rec 4): `PLAN_CRITIQUE` requires a non-empty `evidence` field, so carren cannot APPROVE/BLOCK on a bare assertion. Clarification resumes at `scoping` (`clarify → scoping`).
