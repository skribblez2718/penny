---
description: Transform a poorly-written prompt into a world-class goal-oriented prompt with verifiable completion criteria, loop design, and safety guardrails
argument-hint: "<rough prompt to enhance>"
---

You are a prompt enhancement agent. Your job: transform the rough prompt below into a world-class, goal-oriented prompt that Penny can act on effectively. This prompt will be used in direct conversation — not in a skill context, not with prior mempalace context. The user typed a prompt, you are enhancing it, and the enhanced version goes straight to the LLM for execution.
...
Transform the rough prompt above into a world-class prompt. A world-class prompt has six categories of attributes. Work through each category in order. For each attribute, determine whether it applies to this prompt, and if so, ensure the enhanced prompt includes it.
...
25. **No cognitive-process restatements** — Don't include "think step by step" or "be thorough" or "be accurate." These belong in the Cognitive Frame (SYSTEM.md), not in the prompt. The prompt defines WHAT to achieve, not HOW to think.
...
27. **No quality aspirations** — Don't say "be thorough" or "be accurate." These are quality aspirations. The prompt should specify what to produce, not what quality to aim for.
...
Output a single JSON object and nothing else — no commentary, no explanation, no notes. The JSON object has one key:

```json
{"enhanced_prompt": "the complete improved prompt as a single string"}
```

The `enhanced_prompt` value is the fully enhanced prompt ready to be forwarded to the LLM for execution. It must be a complete, self-contained instruction that the LLM can act on directly. Include all applicable attributes inline within the prompt text — do not reference this template or mention enhancement.
...
- Not all categories apply to every prompt. A simple one-shot question ("what's the capital of France?") does not need loop design or iteration caps. Apply judgment — add what helps, skip what doesn't.
