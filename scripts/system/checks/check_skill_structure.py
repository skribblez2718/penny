...
# Canonical structure (relative to skill root)
REQUIRED_DIRS = ["scripts", "tests", "assets/prompts", "resources"]
REQUIRED_FILES = [
    "SKILL.md",
    "README.md",
    "requirements.txt",
...
def check_skill(skill_dir: Path) -> List[Tuple[str, str]]:
    ...
    if not is_delegate:
        ...
        if scripts_dir.exists() and scripts_dir.is_dir():
            ...
            if misplaced_tests:
                issues.append(("ERROR", f"Test files found in scripts/: {[f.name for f in misplaced_tests]} — move to tests/"))
    ...
    if skill_md.exists():
        ...
        if not content.startswith("---"):
            ...
            if name_match:
                ...
                if not re.match(r"^[a-z0-9]([a-z0-9-]*[a-z0-9])?$", declared_name):
                    issues.append(("ERROR", f"SKILL.md name '{declared_name}' contains invalid characters (must be lowercase a-z, 0-9, hyphens only)"))
                elif declared_name != name:
                    issues.append(("ERROR", f"SKILL.md name '{declared_name}' does not match directory name '{name}'"))
            else:
                issues.append(("ERROR", "SKILL.md: could not parse name field"))

            # Validate description follows canonical pattern: "[sentence]. Use for [cases]. Do not use for [anti-cases]."
            desc_match = re.search(r"^description:\s*(.+)", content, re.MULTILINE)
            if desc_match:
                desc = desc_match.group(1).strip().strip('"')
                if "use for" not in desc.lower():
                    issues.append(("ERROR", "SKILL.md description missing 'Use for' — must follow: '[sentence]. Use for [cases]. Do not use for [anti-cases].'"))
                if "do not use for" not in desc.lower():
                    issues.append(("ERROR", "SKILL.md description missing 'Do not use for' — must follow: '[sentence]. Use for [cases]. Do not use for [anti-cases].'"))
        ...
        required_sections = {
            ...
            "Storing Learnings": r"^##\s+Storing Learnings",
        }
        ...
        if "parallel_phases:" not in content:
            issues.append(("INFO", "SKILL.md: no parallel_phases metadata — consider adding for skills with independent phases"))

    # Info: check orchestrator uses invoke_agents_parallel (not required, but recommended)
    orchestrate = skill_dir / "scripts" / "orchestrate.py"
    if orchestrate.exists():
        orch_content = orchestrate.read_text(encoding="utf-8")
        if "invoke_agents_parallel" not in orch_content:
            issues.append(("INFO", "orchestrate.py: no invoke_agents_parallel — consider parallelizing independent phases"))
...
def main():
    ...
    if total_errors == 0 and total_warnings == 0:
        ...
        print(f"Results: {total_errors} error(s), {total_warnings} warning(s) across {len(skills)} skill(s).")
