...
def aggregate_outcomes(outcomes: List[Dict[str, Any]], include_domains: bool = False) -> Dict[str, Any]:
...
def build_digest_json(
    ...
    for o in outcomes:
        ...
        if sid:
            session_ids.add(sid)
    for d in diary:
        # Diary format varies; extract session_id if present in content
        content = d.get("text", "")
        # Simple heuristic: look for session= or session_id in text
        import re
        for match in re.finditer(r"session[-_]?(?:id)?[:=]?\s*([\w-]+)", content, re.IGNORECASE):
    ...
    sessions_count = len(set(d.get("timestamp", "").split("T")[0] for d in diary)) if diary else len(session_ids)
    ...
    digest = {
        ...
        "amendments_summary": amendments or {"proposed": 0, "approved": 0, "rejected": 0, "pending": 0},
        "signals_summary": {
            "critical_pending": sum(1 for s in (signals or []) if s.get("priority") == "CRITICAL" and s.get("status") == "PENDING"),
            "info_pending": sum(1 for s in (signals or []) if s.get("priority") != "CRITICAL" and s.get("status") == "PENDING"),
        },
        "recommendations": [],
    }
