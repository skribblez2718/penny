...
def render_digest_markdown(digest: Dict[str, Any]) -> str:
    ...
    total_evaluated = (
        outcomes.get("MATCH", 0) + outcomes.get("PARTIAL", 0) + outcomes.get("MISMATCH", 0)
    )
    lines.append("## Outcomes")
    lines.append(
        f"- MATCH: {outcomes.get('MATCH', 0)} ({_safe_percent(outcomes.get('MATCH', 0), total_evaluated)})"
    )
    lines.append(
        f"- PARTIAL: {outcomes.get('PARTIAL', 0)} ({_safe_percent(outcomes.get('PARTIAL', 0), total_evaluated)})"
    )
    lines.append(
        f"- MISMATCH: {outcomes.get('MISMATCH', 0)} ({_safe_percent(outcomes.get('MISMATCH', 0), total_evaluated)})"
    )
    lines.append("")
