...
class TestLedgerWriteReadRoundtrip:
    ...
    def _make_reader(self, store):
        ...
        def reader(query, wing, room, limit):
            ...
            for entry in store:
                ...
                if (
                    query in ("outcome ledger recent", "outcome ledger pending")
                    or query in text
                ):
    ...
    def test_summarize_by_domain(self):
        records = [
            OutcomeRecord("d1", "s", "A", "E", actual_outcome="Done", delta_score="MATCH", domain="coding"),
            OutcomeRecord("d2", "s", "A", "E", actual_outcome="Fail", delta_score="MISMATCH", domain="coding"),
            OutcomeRecord("d3", "s", "A", "E", domain="coding"),  # pending
            OutcomeRecord("d4", "s", "A", "E", actual_outcome="Done", delta_score="MATCH", domain="planning"),
        ]
