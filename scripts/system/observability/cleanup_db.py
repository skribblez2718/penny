...
async def main_async(dry_run: bool = False) -> dict:
    ...
    if dry_run:
        ...
        print(
            f"[DRY RUN] Would delete compactions older than {Config.RETENTION_COMPACTION_DAYS} days"
        )
        print(f"[DRY RUN] Would delete logs older than {Config.RETENTION_LOG_DAYS} days")
        print(
            f"[DRY RUN] Would delete watcher logs older than {Config.RETENTION_WATCHER_LOG_DAYS} days"
        )
        await db.close()
    ...
    if db_size_mb_after / 1024 > Config.DB_SIZE_MAX_GB:
        print(
            f"WARNING: DB size ({db_size_mb_after / 1024:.2f} GB) exceeds threshold ({Config.DB_SIZE_MAX_GB} GB)"
        )
