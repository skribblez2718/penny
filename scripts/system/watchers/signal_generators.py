...
def _has_delta(text: str, score: str) -> bool:
    """Check if outcome text contains a specific delta_score."""
    return bool(re.search(rf"delta_score:\s*{re.escape(score)}", text))
...
def write_signal(signal: dict, session_id: str = "") -> Optional[str]:
    ...
    if result.get("success"):
        ...
        info(
            ...
            data={"drawer_id": drawer_id, "signal_type": signal.get("signal_type"), "priority": signal.get("priority")},
        )
        return drawer_id
...
def generate_mismatch_rate_signal(
    ...
    search = tool_smart_search({
        ...
        "room": "outcomes",
        "limit": 50,
        "include_full": False,  # summaries are enough for delta_score check
    })
    results = search.get("results", [])
    if not results:
        debug("mismatch_rate_watcher", "No outcome results found for MISMATCH query", session_id=session_id)
        return None
    ...
    for r in results:
        text = r.get("summary", r.get("text", ""))
        if "delta_score" not in text and "confidence_at_action" not in text:
            continue  # skip schema/protocol drawers
        total_relevant += 1
        dt = _parse_dt(text)
        if dt and dt < cutoff:
            continue
        if _has_delta(text, "MISMATCH"):
            mismatch_count += 1
    ...
    info(
        ...
        session_id=session_id,
        data={"mismatch_count": mismatch_count, "total_relevant": total_relevant, "threshold": threshold},
    )
...
def generate_confidence_trend_signal(
    ...
    for r in results:
        text = r.get("summary", r.get("text", ""))
        if "confidence_at_action" not in text:
            continue
        total += 1
        dt = _parse_dt(text)
        if dt and dt < cutoff:
...
def generate_task_staleness_signal(
    ...
    search = tool_smart_search({
        "query": "PARTIAL MISMATCH unresolved",
        "wing": "penny",
        "room": "outcomes",
        "limit": 50,
        "include_full": False,
    ...
    results = search.get("results", [])
    if not results:
        debug("task_staleness_watcher", "No outcome results found for staleness query", session_id=session_id)
        return None
    ...
    for r in results:
        text = r.get("summary", r.get("text", ""))
        m = re.search(r"decision_id:\s*(\S+)", text)
        if not m:
            continue
        did = m.group(1)
        dt = _parse_dt(text)
        if not dt:
            continue
        if _has_delta(text, "MATCH"):
            newest_match[did] = max(newest_match.get(did, dt), dt)

    # Now count PARTIAL/MISMATCH outcomes whose decision_id has no newer MATCH
    stale = 0
    for r in results:
        text = r.get("summary", r.get("text", ""))
        m = re.search(r"decision_id:\s*(\S+)", text)
        ...
        if not (_has_delta(text, "PARTIAL") or _has_delta(text, "MISMATCH")):
            continue
        # Stale if no MATCH outcome exists after this PARTIAL/MISMATCH
        match_dt = newest_match.get(did)
        if match_dt is None or match_dt <= dt:
            stale += 1
...
def run_all_metric_watchers(session_id: str) -> list[str]:
    ...
    for fn in funcs:
        ...
        try:
            ...
            sig = fn(session_id)
            if sig:
                debug("ambient_watchers", f"Watcher {watcher_name} generated signal {sig.get('signal_id')}", session_id=session_id)
                ...
                if drawer_id:
                    ...
                    info(
                        ...
                        session_id=session_id,
                        data={"watcher": watcher_name, "drawer_id": drawer_id, "priority": sig.get("priority")},
                    )
                else:
                    warn(
                        "ambient_watchers",
                ...
                debug("ambient_watchers", f"Watcher {watcher_name} produced no signal", session_id=session_id)
        except Exception as exc:
            exception("ambient_watchers", f"Watcher {watcher_name} failed", exc, session_id=session_id)
...
if __name__ == "__main__":
    ...
    try:
        ...
        info("ambient_watchers", "Watcher process finished successfully", session_id=sid, data=output)
