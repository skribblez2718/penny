...
def test_pi_json_call_returns_last_assistant_text():
    out = pi_json_call(
        "p", model_spec="anthropic/haiku", system="s", runner=_runner(_stream('{"answer": 1}'))
    )
    assert out == '{"answer": 1}'
...
def test_pi_json_call_none_on_all_failure_modes():
    ...
    assert (
        pi_json_call(
            ...
            model_spec="m",
            system="s",
            runner=_runner(raise_exc=subprocess.TimeoutExpired("pi", 1)),
        )
        is None
    )
    assert (
        pi_json_call("p", model_spec="m", system="s", runner=_runner(raise_exc=OSError("x")))
        is None
    )
...
def test_detect_success_and_confidence_normalized():
    payload = json.dumps(
        {"answer": "coding", "evidence": ["uses uv.lock"], "confidence": "certain"}
    )
    r = detect(
        "uv.lock present",
        "what domain?",
        model_spec="anthropic/haiku",
        labels=["coding", "planning"],
        runner=_runner(_stream(payload)),
    )
    assert r == {
        "ok": True,
        "answer": "coding",
        "evidence": ["uses uv.lock"],
        "confidence": "CERTAIN",
    }
...
def test_detect_bad_confidence_defaults_probable():
    r = detect(
        ...
        "q",
        model_spec="m",
        runner=_runner(_stream(json.dumps({"answer": "x", "confidence": "meh"}))),
    )
