...
_UNIVERSAL_KEYWORDS = frozenset([
    # Security directives
    "security directive", "system_directive", "system_boundary",
    "agent_boundary", "immutable",
    # Cognitive Frame sections
    "before responding", "before_responding",
    "instruction hierarchy", "instruction_hierarchy",
    "self-verification", "self_verification", "self verification",
    "canonical vocabulary", "canonical_vocabulary",
    "reasoning style", "reasoning_style",
    # Confidence levels
    "confidence level", "confidence_level", "CERTAIN", "PROBABLE", "POSSIBLE", "UNCERTAIN",
    "new confidence",
    # Core rules
    "truth", "safety", "clarity", "thoroughness",
    "never fabricate", "always verify",
    "mission rule", "output contract",
    "limitations", "restrictions", "boundaries",
    "options", "parameters", "choices",
    "guesses", "expectations", "defaults",
    "gaps", "questions", "uncertainties",
    "compromises", "costs", "sacrifices",
    "validation", "testing",
])
...
_PREFERENCE_KEYWORDS = frozenset([
    ...
    "user consistently", "user often", "user asked",
    "communication style", "concise", "verbose", "terse",
    "confirmation threshold", "ask before", "always confirm",
    "workflow preference", "work style",
])

# Config keywords — operational changes
_CONFIG_KEYWORDS = frozenset([
    "timeout", "directory", "path", "environment variable",
    "config value", ".env", "setting",
    "increase timeout", "decrease timeout",
    "global path", "relative path",
])

# Domain-guidance keywords — skill/agent name + domain action
_DOMAIN_KEYWORDS = frozenset([
    "piper", "echo", "carren", "tabitha", "plan skill",
    "exploration", "critique", "taskify",
    "coding", "testing", "refactor", "explore",
    "config file", "package manager",
    "CREST", "summary format", "output format",
    "skill prompt", "domain evaluation",
    "underestimates", "overestimates", "misses",
    "incorrectly assumes", "wrong package",
])
