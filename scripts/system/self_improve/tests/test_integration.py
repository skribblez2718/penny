...
class TestCompressionToPipeline:
    ...
    def test_compression_output_valid_for_all_modules(self):
        outcomes = [
            {"decision_id": "d1", "outcome": "MISMATCH", "domain": "coding", "reason": "assumed uv without checking", "session_id": "s1"},
            {"decision_id": "d2", "outcome": "MISMATCH", "domain": "coding", "reason": "assumed uv without checking", "session_id": "s1"},
        ]
        ...
        assert a["target_layer"] in ("DOMAIN_GUIDANCE", "MEMPALACE_PREF", "CONFIG", "REJECTED_UNIVERSAL")

    def test_compression_with_universal_pattern_rejected_at_applier(self):
        outcomes = [
            {"decision_id": "d1", "outcome": "MISMATCH", "domain": "universal", "reason": "Add before responding step", "session_id": "s1"},
            {"decision_id": "d2", "outcome": "MISMATCH", "domain": "universal", "reason": "Add before responding step", "session_id": "s1"},
        ]
        amendments = run_compression_loop(outcomes)
        assert len(amendments) == 1
