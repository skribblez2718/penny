...
Usage:
  python3 scripts/system/register_artifact.py agent \
    --name compliance \
    --description "Audit work products against Penny standards" \
    --file-path .pi/agents/compliance.md \
    --purpose "Evaluate agent definitions, skills, and plans for compliance" \
  ...
  python3 scripts/system/register_artifact.py skill \
    ...
    --description "Analyze weather data patterns and generate reports" \
...
class AgentsMdUpdater:
    ...
    def update_feature_index(self, spec: ArtifactSpec) -> Tuple[bool, str]:
        ...
        if spec.artifact_type == "agent":
            ...
            human_doc = f"`docs/humans/capabilities/{spec.name}/{spec.name}.md`"
            agent_doc = f"`docs/agents/capabilities/{spec.name}/{spec.name}.md`"
            design = f"`.pi/skills/{spec.name}/README.md`" if not spec.design_doc else f"`{spec.design_doc}`"
            impl = f"`.pi/skills/{spec.name}/`" if not spec.implementation_dir else f"`{spec.implementation_dir}`"
...
class RegisterArtifact:
    ...
    def register(self, spec: ArtifactSpec) -> RegistrationResult:
        result = RegistrationResult(success=False, agents_md_updated=False, human_doc_created=False, agent_doc_created=False, links_valid=False)
