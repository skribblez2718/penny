...
for pkg in fastapi uvicorn aiosqlite python-dotenv pydantic apscheduler; do
    if PYTHONPATH="$OBS_DIR/src" python -c "import $pkg" 2>/dev/null; then
        echo -e "${GREEN}  ✓ $pkg${NC}"
    else
...
if [ "$DEPS_OK" = false ]; then
    echo -e "${RED}✗ Some dependencies are missing. Run 'make install-py' or 'uv sync'.${NC}"
    exit 1
fi

if PYTHONPATH="$OBS_DIR/src" python -c "from observability.main import app; print('OK')" 2>/dev/null; then
    echo -e "${GREEN}✓ Observability imports verified${NC}"
else
    echo -e "${RED}✗ Import verification failed. Run 'make install-py' or 'uv sync'.${NC}"
    exit 1
fi
...
echo "  cd $PROJECT_ROOT && PYTHONPATH=$OBS_DIR/src python -m observability"
echo ""
