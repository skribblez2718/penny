...
PYTHON_PKGS=(
    "tree-sitter:jsa splitter (AST parsing)"
    "tree-sitter-javascript:jsa splitter (JS grammar)"
    "tiktoken:jsa splitter (token counting, recommended)"
    "python-docx:resume extension (.docx export)"
    "lxml:resume extension (XML processing)"
)
...
if [ "$SKIP_PYTHON" = true ]; then
    ...
    for entry in "${PYTHON_PKGS[@]}"; do
        ...
        if python -c "import ${name//-/_}" 2>/dev/null; then
            ok "$name ($desc) — already installed"
            PASSED+=1
            continue
        fi
