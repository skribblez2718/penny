...
2. **Progress-assessment gate (loop-research Recs 1 & 2).** `progress_check(state, ctx, summary)` runs after the UNCERTAIN check, before routing. If it returns a reason string — a retry whose `strategy_change` repeats a prior approach (`strategy_repeated`), or N iterations with the same unresolved gaps (`is_stalled`) — and the state is escalatable, the engine escalates with that reason as the `unknown_reason`. This bounds a spinning loop instead of burning the remaining `ctx.max_iterations` budget.
