...
| `investigate` | AGENT (loop) | `annie` | Bounded wave loop — `total_waves = max(1, ceil(needs_llm / WAVE_SIZE))`, runs at least one wave (the general sweep). `route_after` self-transitions (`investigate_wave`) until waves exhaust, then `investigate_done` fires straight into `collect` (no gate). Reports `unverified_count` honestly |
