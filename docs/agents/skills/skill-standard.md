...
2. **Verify is evidence-gated.** Any VERIFY/VALIDATE/critique state declares an `evidence` field in its contract (`_c(required={..., "evidence": list}, evidence=["evidence"])`) so the engine rejects a PASS carrying no captured evidence (Rec 3). The verifier prompt states the evidence-tier hierarchy (execute > apply-the-rule > judge); a PASS that could have been executed but was only judged is under-verified. The captured evidence rides to `ctx.verify_evidence`.
...
4. **Directives carry THIS run's facts only — never retrieved memory.** A `_task_summary` (base or override) is built from the goal, constraints, and per-state run facts. Do **not** query MemPalace and prepend the result to an agent directive. Agent context comes from `.pi/agents/<agent>.md` plus skill orchestration. Agents that need stored context read it themselves, with their own tools, from their session room.
...
└── resources/
    ...
    └── flow.html              # State diagram mirroring machine_cls (self-contained dark HTML —
                               #   THE standard; drift-guarded by test_flow_diagrams.py)
    # The checker requires resources/ to exist and a flow diagram. resources/flow.html
    # is THE standard (see flow-diagrams.md); the legacy Mermaid resources/flow.mmd is
    # still accepted for not-yet-migrated skills but is deprecated — convert it and
    # delete the .mmd. All OTHER resource filenames (reference, rubric, etc.) are
    # conventional — name them freely.
```
...
- [ ] **Compliance (see the section above):** no keyword router / magic-number table; VERIFY states are evidence-gated; fan topology is model-emitted (fixed topology only as a tagged LOAN); `_task_summary` overrides keep directives limited to current-run facts; honest exhaustion on budget spend; the dial is documented; every kept heuristic is a tagged LOAN in `orchestration/loans.py`
