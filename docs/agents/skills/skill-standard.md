...
4. **Directives carry THIS run's facts only — never retrieved memory.** A `_task_summary` (base or override) is built from the goal, constraints, and per-state run facts. Do **not** query MemPalace and prepend the result to an agent directive. The engine previously did this ("Recall", atom F2, `orchestration/recall.py`): it searched `penny/system_amendments` with no status filter, so `PENDING` and even operator-`REJECTED` amendment proposals were rendered into agent prompts — bypassing the approval gate that governs amendment *application*. The mechanism is removed and `tests/test_no_memory_injection.py` fails any reintroduction. Agent context comes from `.pi/agents/<agent>.md` plus skill orchestration; an amendment changes behaviour only by being **applied to a file after explicit human approval**. Agents that need stored context read it themselves, with their own tools, from their session room.
