...
front-load global decisions behind a gate, and draw `resources/flow.html`.
...
6. **Create the checker-required files:** `README.md` (include the order-rule → failure-mode table), `resources/reference.md` (FSM/contracts/constraints reference mirroring the playbook), `resources/flow.html` (state diagram).
...
- [ ] `README.md`, `resources/reference.md`, `resources/flow.html` present (checker-enforced)
