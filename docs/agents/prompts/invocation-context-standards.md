...
The plan skill's orchestrator (`orchestrate.py`) constructs task messages via the `task_summary` field in its JSON action output. Example:

```python
action = {
    "action": "invoke_agent",
    "agent": "echo",
    "task_summary": "Explore for session plan-001. Goal: Refactor auth module. "
                    "Mempalace room: skills/plan-plan-001",
    "orchestrator_state": {...}
}
```

The `task_summary` becomes the user message after `<system_boundary>`. It must be:
...
1. **Task message template changes**: Modify `orchestrate.py` task summary generation. Test with a single session before deploying.
