...
### Circumstance 3: Direct Conversation with Prompt Improver (Future)

User types a message, but a `before_agent_start` flow intercepts and enhances it before Penny sees it.
...
**The improver's own prompt assembly:**

```
[Improver agent's system prompt]:
  Cognitive Frame          ← SYSTEM.md
  Role Definition           ← .pi/agents/improver.md (or Penny acting as improver)
  Domain Guidance           ← assets/prompts/improve.md (improvement methodology)
  Project Index              ← AGENTS.md files

[Improver agent's user message]:
  "Original prompt: {raw_user_input}. Improve this prompt per your domain guidance."
```

The improver's output replaces the raw user prompt in the main Penny's Invocation Context.

### Circumstance 4: Skill Invocation with Prompt Improver (Future)
...
| **orchestrate.py**       | State machine; outputs JSON actions for Penny to route | Constructs Invocation Context (task messages with goal + session ID). Reads/writes mempalace for inter-agent state. |
...
#### The Prompt Improver (Future)

A `before_agent_start` flow that intercepts and enhances the raw user prompt before Penny processes it.

| Component          | What It Provides                                              | Relationship to Layers                                                                                                                                                                                                 |
| ------------------ | ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Improver agent** | Transforms raw user prompt into structured Invocation Context | _Pre-processes_ Invocation Context. Uses its own Cognitive Frame + Role Definition + Domain Guidance (it's a separate agent invocation). Its **output** becomes the Invocation Context for the main Penny interaction. |

The improver is not a new layer. It's a **transformation pipeline** that takes raw user input and produces better-structured Invocation Context. The main Penny interaction receives the improved Invocation Context and doesn't know (or need to know) that a transformation occurred.
