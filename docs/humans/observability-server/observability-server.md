...
Beyond logs, the server archives compaction artifacts and mirrors engine runs:

- `POST /compactions` (ingest, from the compaction extension) · `GET /sessions/{session_id}/compactions` · `GET /sessions/{session_id}/compactions/{compaction_seq}` — the archive holds the full structured artifact; model context only receives its prose brief + `[RESUME-REFS]` appendix.
- `POST /orchestration/runs` · `POST /orchestration/events` (ingest, from the engine's ObsClient) · `GET /orchestration/runs[?session_id=&status=]` · `GET /orchestration/runs/{run_id}` · `GET /orchestration/runs/{run_id}/events` · `GET /sessions/{session_id}/orchestration` — reporting mirror of the engine's local checkpointer DB (`.penny/orchestration.db`), which remains the source of truth.
