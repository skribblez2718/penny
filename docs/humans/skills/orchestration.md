...
| **Orchestrator** | Manages the state machine and emits action directives. | The Python process in `scripts/orchestrate.py`. |
...
## What `orchestrate.py` Does

The orchestrator file has three jobs:
...
3. **Serialize.** Between calls, the orchestrator writes its state to a temporary session file so it can resume if interrupted.
