...
| §1 Before Responding protocol (mandatory cognitive steps) | **[DEBUNKED-ADJACENT]** | Prescriptive step scaffolds are the technique class that goes neutral-to-negative on reasoning-native models. Survives only as *lightweight* directives; monitored per family by the degradation gate (`prompt_efficacy.frame_regressed_families`). |
...
| §9 Self-verification as structured attention, NOT a correctness audit | **[EVIDENCE]** for the framing | Intrinsic self-correction is debunked (models are poor at catching their own errors) — which is exactly why the Delivery Checklist only checks *presence of required elements*, and why correctness review routes to a different model (Carren/Vera). |
