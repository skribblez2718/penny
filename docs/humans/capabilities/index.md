...
| [Judgment Calibration](judgment-calibration/judgment-calibration.md) | Freezes Fable's PASS/FAIL judgment into a corpus and picks an open-model verifier that reproduces it, so quality survives the driver-model downgrade. |
...
| [Prompt Improver](prompt-improver/prompt-improver.md) | Rewrites raw prompts into better-specified Invocation Context — goal restated, facts preserved, blockers surfaced as open questions — with user confirmation. |
