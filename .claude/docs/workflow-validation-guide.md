...
grep -E "version|compatible|install" .claude/memory/task-*-research-discovery-memory*.md | head -20

# Check synthesis agent documented compatibility
grep -E "compatibility|conflict|version" .claude/memory/task-*-synthesis-agent-memory*.md | head -20
```
