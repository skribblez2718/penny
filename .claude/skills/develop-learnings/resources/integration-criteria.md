...
**Examples:**
- ✅ CORE (generation-agent): "Use HTTP/SSE transport for MCP servers" - core to generating functional servers
- ❌ PERIPHERAL (generation-agent): "Consider adding telemetry for observability" - valuable but not core to generation function
...
- Target File: `.claude/skills/develop-mcp-server/SKILL.md`
...
**Example:** "Use connection pooling for database clients" might apply to 90% of database-related tasks, but only 40% of generation-agent's overall tasks → STANDALONE
...
- CORE for generation-agent → INTEGRATE
- PERIPHERAL for research-agent → STANDALONE + reference
